...
let ServerTest = class
{
  ...
  validateResponseObject(test, response, index, got, expected)
  {
  ...
  testAll()
  {
    this._tests.forEach(
      function(test)
      {
        let             jsonResponse;
        let             responses;
        let             expected = JSON.stringify(test.responses);
   ...
   */
  _serviceFactory(methodName, error)
  {
    let f = this._services[methodName];
   ...
   */
  registerService(serviceName, fService, paramNames)
  {
    let             f;
